...
# 폰트 문제와 레이아웃을 모두 해결하는 centered 설정
...
# 1. 데이터 로드 및 전처리
@st.cache_data
def load_trade_data(file_path, mode):
    ...
    # [수정 포인트] '시점'에서 '년'을 삭제하여 가로 공간 확보
    # 연도별: '2010년' -> '2010'
    # 분기별: '2010년 1분기' -> '2010 1분기'
...
try:
    df = load_trade_data(file_name, data_mode)
    ...
    if target_metrics:
        ...
        # use_container_width=False로 설정하면 창을 넓혀도 그래프가 무한정 커지지 않습니다.
        # 하지만 중앙 정렬(layout="centered") 모드이므로 True로 두어도 정해진 폭까지만 커집니다.
